...
#include "unconditional286_vb_sse.hpp"
#include "unconditional286_vb_serial.hpp"
...
class Unconditional286CompressorHeaderSSE
{
public:
    Unconditional286CompressorHeaderSSE(
    ...
    static Unconditional286CompressorHeaderSSE decode(const uint8_t *ptr) noexcept
    {
        ...
        return Unconditional286CompressorHeaderSSE(n, first_byte_compressed_size, second_byte_compressed_size);
    }
...
class Unconditional286CompressorSSE
{
public:
    ...
    static uint64_t encode(
        ...
        SIMDAcceleration accelerator = SIMDAcceleration::SSE);
    ...
    static Unconditional286CompressorHeaderSSE get_header(uint8_t * ptr)
    {
        return Unconditional286CompressorHeaderSSE::decode(ptr);
    }
...
>
inline uint64_t Unconditional286CompressorSSE::encode(
    const int16_t parallel_nanopore_restrict *samples,
    const size_t number_of_samples,
    uint8_t parallel_nanopore_restrict *output,
    SIMDAcceleration accelerator)
{
    uint8_t *const noise_data_ptr = output + Unconditional286CompressorHeaderSSE::size();
    ...
    if (accelerator == SIMDAcceleration::SSE)
    {
    ...
    {
        Unconditional286VbSerial::encode<AllowReorder>(
            reinterpret_cast<const uint16_t *>(vb_input_samples),
            number_of_samples,
            noise_data_ptr,
            first_byte_data_ptr,
    ...
    const auto first_byte_compression_result = ZSTD_compress(
        output + Unconditional286CompressorHeaderSSE::size() + Unconditional286VbSizes::small_data_buffer_size_bytes(number_of_samples),
    ...
    {
        unconditional286compressor_cleanup_routine(first_byte_data_ptr, second_byte_data_ptr);
        return -1;
    }

    const auto second_byte_compression_result = ZSTD_compress(
        output + Unconditional286CompressorHeaderSSE::size() + Unconditional286VbSizes::small_data_buffer_size_bytes(number_of_samples) + first_byte_compression_result,
    ...
    Unconditional286CompressorHeaderSSE hdr(number_of_samples, first_byte_compression_result, second_byte_compression_result);
    hdr.encode(output);
    ...
    return
        ...
        Unconditional286CompressorHeaderSSE::size();
}
...
template<bool AllowReorder,
    ...
    int ZSTDCompressionLevel
    >
inline void Unconditional286CompressorSSE::decode(
    const uint8_t parallel_nanopore_restrict *bytes,
    int16_t parallel_nanopore_restrict *output,
    SIMDAcceleration accelerator)
{
    const auto hdr = Unconditional286CompressorHeaderSSE::decode(bytes);
    const auto number_of_samples = hdr.number_of_samples();

    const uint8_t parallel_nanopore_restrict *noise_data_ptr = bytes + Unconditional286CompressorHeaderSSE::size();
...
//FIXME: delta encode and rice encode!!!
    ...
    const auto first_byte_decompression_result = ZSTD_decompress(
        first_byte_data_ptr,
        Unconditional286VbSizes::medium_data_buffer_size_bytes(number_of_samples),
        bytes + Unconditional286CompressorHeaderSSE::size() + Unconditional286VbSizes::small_data_buffer_size_bytes(number_of_samples),
    ...
    const auto second_byte_decompression_result = ZSTD_decompress(
        second_byte_data_ptr,
        Unconditional286VbSizes::large_data_buffer_size_bytes(number_of_samples),
        bytes + Unconditional286CompressorHeaderSSE::size() + Unconditional286VbSizes::small_data_buffer_size_bytes(number_of_samples) + hdr.first_bytes_data_compressed_size(),
...
size_t Unconditional286CompressorSSE::encode_bound(size_t number_of_samples)
{
    ...
    return Unconditional286CompressorHeaderSSE::size() + small_data_bytes + medium_data_bytes + large_data_bytes;
}
